...
export default async function PatientDiaryPage({
  ...
  // 1. Extraímos os tipos diretamente da constante entries gerada pelo Prisma
  type EntryType = (typeof entries)[number];
  type CommentType = EntryType["comments"][number];
    ...
    <div className="space-y-6">
      ...
      {entries.length === 0 ? (
        ...
        <div className="space-y-4">
          {/* 2. Adicionamos a tipagem EntryType aqui */}
          {entries.map((entry: EntryType) => (
            <Card key={entry.id}>
              <CardContent className="pt-6 space-y-4">
                <div className="flex items-center justify-between">
                  <p className="text-sm font-medium text-gray-900">
                    {format(new Date(entry.date), "EEEE, dd 'de' MMMM", {
                      locale: ptBR,
                    })}
                  </p>
                  {entry.mood && (
                ...
                {entry.comments.length > 0 && (
                  <div className="space-y-2">
                    {/* 3. Adicionamos a tipagem CommentType aqui para prevenir o mesmo erro */}
                    {entry.comments.map((comment: CommentType) => (
                      <div
                        key={comment.id}
                ...
                <DiaryCommentForm
                  entryId={entry.id}
                  hasComment={entry.comments.length > 0}
                />
              </CardContent>
            </Card>
          ))}
