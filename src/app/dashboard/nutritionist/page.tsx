...
import { Users, ClipboardList, Camera, TrendingUp, AlertCircle } from "lucide-react"
...
export default async function NutritionistDashboard() {
  ...
  const activeToday = patients.filter(
    (p) => p.mealLogs.length > 0 || p.diaryEntries.length > 0 || p.dailyQuiz.length > 0
  ).length
  ...
  const totalPending = pendingMealComments + pendingDiaryComments + pendingQuizFeedback
    ...
    <div className="space-y-6">
        ...
        <Card>
          <CardContent className="pt-6">
            <div className="flex items-center gap-3">
              ...
              <div>
                <p className="text-2xl font-bold text-gray-900">{pendingQuizFeedback}</p>
      ...
      <Card>
        ...
        <CardContent className="p-0">
          {patients.length === 0 ? (
            <p className="text-gray-500 text-sm p-6">Nenhum paciente vinculado ainda.</p>
          ) : (
            <ul className="divide-y divide-gray-100">
              {patients.map((patient) => {
                const mealsToday = patient.mealLogs.length
                const quizToday = patient.dailyQuiz[0]
                const diaryToday = patient.diaryEntries.length
                const pendingPatient =
                  patient.mealLogs.filter((m) => m.comments.length === 0).length +
                  patient.diaryEntries.filter((d) => d.comments.length === 0).length +
                ...
                return (
                  <li key={patient.id}>
                    ...
                    >
                      ...
                      <div className="flex items-center gap-2 md:gap-4 text-xs text-gray-500 flex-shrink-0">
                        <span className="flex items-center gap-1">
                          <Camera className="w-3.5 h-3.5" />
                          <span className="hidden sm:inline">{mealsToday} hoje</span>
                          <span className="sm:hidden">{mealsToday}</span>
                        </span>
                        <span className="flex items-center gap-1">
                          <ClipboardList className="w-3.5 h-3.5" />
                          {quizToday ? `${quizToday.dailyNote.toFixed(1)}` : "—"}
                        </span>
                        {pendingPatient > 0 && (
                          <span className="bg-orange-100 text-orange-600 font-semibold px-2 py-0.5 rounded-full whitespace-nowrap">
                            {pendingPatient}
                            <span className="hidden md:inline"> pendente{pendingPatient > 1 ? "s" : ""}</span>
                          </span>
                        )}
                      </div>
                    </Link>
